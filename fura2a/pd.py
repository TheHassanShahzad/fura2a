...
class JointStateProcessor(Node):
    ...
    def pd_control(self, pendulum_position, pendulum_velocity):
        # Desired position for the pendulum at the inverted position is pi
        desired_position = pi
        
        # Calculate shortest-path error in position
        error_position = self.normalize_angle(desired_position - pendulum_position)
        error_velocity = 0.0 - pendulum_velocity

        # PD control law
        control_effort = self.Kp * error_position + self.Kd * error_velocity
    ...
    def listener_callback(self, msg):
        ...
        try:
            ...
            if pi - abs(pendulum_position) <= self.position_threshold: 
                ...
                self.effort_command = self.pd_control(pendulum_position, pendulum_velocity)
            else:
