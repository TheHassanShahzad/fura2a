...
class JointStateProcessor(Node):
    ...
    def listener_callback(self, msg):
        ...
        try:
            ...
            pendulum_position = self.limit_minus_pi_pi(msg.position[pendulum_index])
            ...
            self.data[1].append(pendulum_velocity)
            ...
            if self.count == self.max_count:
                pos_list = self.data[0]
                vel_list = self.data[1]

                plt.figure(figsize=(8, 5))
                plt.plot(pos_list, label="Pendulum Position")
                plt.plot(vel_list, label="Pendulum Velocity")
